...
class ValueScanner:
    ...
    def __init__(self, config: dict = CONFIG):
        self.config = config
        # Use FMP v2 stable endpoints (post Aug 2025)
        import sys as _sys, os as _os
        _sys.path.insert(0, _os.path.dirname(_os.path.abspath(__file__)))
        from data_provider_fmp_v2 import FMPv2DataProvider
        fmp_key = config.get("FMP_API_KEY", "")
        self.data_provider = FMPv2DataProvider(fmp_key)
        print("  Using FMP v2 stable endpoints")
