...
BEEHIIV_PUB_ID   = os.getenv("BEEHIIV_PUB_ID", "")   # pub_xxxxxxxx from Beehiiv
...
def send_email_via_beehiiv(picks, scan_date):
    """
    Create and send a broadcast via Beehiiv API.
    Beehiiv docs: https://developers.beehiiv.com/docs/v2/posts-create
    """
    if not BEEHIIV_API_KEY or not BEEHIIV_PUB_ID:
        print("  [Email] Beehiiv not configured — skipping email send")
        print("  [Email] Set BEEHIIV_API_KEY and BEEHIIV_PUB_ID env vars")
        return False
    ...
    post_payload = json.dumps({
        ...
        "send_to":        "free",        # "free" | "premium" | "all"
        "preview_text":   f"Today: {', '.join(p['metrics']['ticker'] for p in picks[:3])} — full research inside",
    }).encode()
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as r:
            ...
            print(f"  [Email] Post created: {post_id}")

        # Step 2: Publish/send the post
        if post_id:
            pub_payload = json.dumps({"send_to": "free"}).encode()
            pub_req = urllib.request.Request(
                f"https://api.beehiiv.com/v2/publications/{BEEHIIV_PUB_ID}/posts/{post_id}/send",
                data=pub_payload,
                headers={
                    "Content-Type": "application/json",
                    "Authorization": f"Bearer {BEEHIIV_API_KEY}",
            ...
            with urllib.request.urlopen(pub_req, timeout=30) as r:
                ...
                print(f"  [Email] Sent — status: {send_result.get('data', {}).get('status')}")
                return True

    except Exception as e:
        print(f"  [Email] Error sending via Beehiiv: {e}")
        return False
...
class ValueScanAPI(BaseHTTPRequestHandler):
    ...
    def _get_status(self):
        ...
        self.send_json({
            ...
            "beehiiv_configured": bool(BEEHIIV_API_KEY and BEEHIIV_PUB_ID),
        })
    ...
    def _handle_scan_complete(self, body):
        ...
        Thread(target=send_email_via_beehiiv, args=(picks, scan_date), daemon=True).start()
    ...
    def _handle_new_subscriber(self, body):
        ...
        try:
            ...
            Thread(target=send_welcome_email, args=(email, name), daemon=True).start()
